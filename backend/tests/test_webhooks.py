...
def test_webhook_targets_are_ssrf_guarded(client, student):
    ...
    behalf, so the default refuses everything the internet cannot route to.
    Without this, any student turns the API into a probe of the lab network
...
def test_subscribe_validate_and_sign(
    ...
    sub = client.post(
        "/api/v1/webhooks",
        json={
            # Loopback with nothing listening: the attempt is made and fails.
    ...
    # Nothing is listening there, so the attempt was made and failed cleanly.
